...
void aplicarFiltro(int *imagen, int *imagenProcesada, int width, int height) {
    // Las máscaras Sobel son constantes y pueden ser compartidas
    int Gx[3][3] = {{-1, 0, 1}, {-2, 0, 2}, {-1, 0, 1}};
    int Gy[3][3] = {{-1, -2, -1}, {0, 0, 0}, {1, 2, 1}};

    // Paralelizar el bucle
    #pragma omp parallel for collapse(2) default(none) \
            shared(imagen, imagenProcesada, width, height, Gx, Gy) \
            schedule(dynamic)
    for (int y = 1; y < height - 1; y++) {
        for (int x = 1; x < width - 1; x++) {
            // Variables locales automáticamente privadas para cada hilo
            int sumX = 0;
            int sumY = 0;
...
int calcularSumaPixeles(int *imagen, int width, int height) {
    ...
    // Paralelizar el bucle usando reduction para la suma
    #pragma omp parallel for reduction(+:suma) default(none) \
            shared(imagen, width, height)
